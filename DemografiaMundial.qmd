...
ggplot(nat, aes(x=Continente, y = Natalidad ,fill = Año)) + 
  geom_boxplot() + 
  geom_point(aes(x = Continente, y = med, fill = Año))+
...
world_population %>% 
  ...
  #geom_point(mapping = aes(x = Continente, y = mean(Esp_vida))) +
  guides(fill = guide_legend(title = "Año")) +
...
world_population %>% 
  filter(Continente == "North America", Año == 2019, Esp_vida == 0)
...
Resalta África; la esperanza de vida es mucho más baja que en los demás continentes
...
ggpairs(a_africa,mapping=ggplot2::aes(colour = Año),
        upper = list(continuous = wrap("cor", size = 3)))
