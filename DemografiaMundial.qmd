...
- $\textit{Densidad}$ que nos indica la densidad de cada país; recordemos que la densidad no és más que un indicador que nos permite saber cuánta población habita en una zona territorial. Ésta viene dada por: 
  $\textit{Densidad} =\frac{\textit{Pob_total}}{\textit{Area_km2}}$
...
world_population %>% 
  ...
  summarise( media = mean(Natalidad))
```
Como vemos, los valores difieren significativamente de los de Europa. 
No vemos diferencias significativas por años en ningún continente.
