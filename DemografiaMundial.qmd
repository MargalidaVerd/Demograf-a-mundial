...
Nos resulta impactante que el país con más mortalidad del mundo sea Bulgaria, sin embargo, tras investigar un poco, hemos observado que se trata del país con más pobreza de la UE, con una inversión en el sistema sanitario muy bajo, lo que imlicó ser el país de la UE con más muertes a lo largo de la pandemia. Añadiendo que la tasa de desempleo es muy alta, tenemos como consecuendia que una tasa de natalidad muy baja, resultando asi en un índice de crecimiento de -1.   
...
Sin embargo, nos llama la atención el valor máximo de Norteamérica, y tras investigar un poco, encontramos que en 2018 hibo un terremoto en Haití, que hizo que las tasas de mortalidad infantil augmentaran respecto al resto de años, sin embargo, en general, las tasas en Haití siguen siendo muy altas, esto viene provocado por la situación de pobreza y de insalubridad, que causan enfermedad y sufrimiento entre la población infantil, un 8% de la cual sufre desnutrición grave y el 22% padece desnutrición crónica.
...
ggplot(fert, aes(x=Continente, y = Fertilidad ,fill = Año)) + 
  geom_boxplot() + 
  geom_point(aes(x = Continente, y = med, colour = "white"))+
  guides(fill = guide_legend(title = "Año")) +
...
```
La esperanza de vida és un claro indicador de la calidad de vida de un determinado país; así, teniendo en cuenta las enfermedades causadas por la falta de agua, la poca asistencia médica y la desnutrición, hacen que Chad sea él país con una esperaza de vida de tan solo 53 años, siendo la más baja del mundo.

De la misma forma, veamo qué país presenta una esperanza de vida más alta:
```{r}
...
p_19 <- data_2019 %>% 
  ...
  arrange(desc(Pob)) %>% 
  mutate(Area = round(Area_km2 / 1000000, 2)) %>% 
  arrange(desc(Area)) %>% 
  mutate(text= paste("Pais: ", Pais, "\nPoblación (M):", Pob, "\nArea_km2: ", Area)) %>%
  ggplot(aes(x=Pob, y = Area, size= Pob_total, color = Continente, text = text)) +
...
ggpairs(a_asia,mapping=ggplot2::aes(colour = Año),
...
ggpairs(a_north_am,mapping=ggplot2::aes(colour = Año),
        upper = list(continuous = wrap("cor", size = 3)))
...
ggpairs(a_oceania,mapping=ggplot2::aes(colour = Año),
        upper = list(continuous = wrap("cor", size = 3)))
...
ggpairs(a_south_am,mapping=ggplot2::aes(colour = Año),
        upper = list(continuous = wrap("cor", size = 3)))
...
corr_esp_mort_europe<- a_europe %>%
  select(Esp_vida,Mortalidad) %>%
  na.omit %>% as.matrix()

View(corr_esp_mort_europe)

bvbox(corr_esp_mort_europe,
      col = c("red", "blue", "green"),
      xlab = "Esperanza vida", 
           ylab = "Mortalidad",
      pch = 19, cex = 1.25)

corr_nat_mort_europe<- a_europe %>%
  select(Natalidad,Fertilidad) %>%
  na.omit %>% as.matrix()

bvbox(corr_nat_mort_europe,
      col = c("red", "blue", "green"),
      xlab = "Natalidad", 
           ylab = "Fertilidad",
      pch = 19, cex = 1.25)
...
#ggplot(a_asia, aes(x = Mortalidad)) + 
...
corr_esp_mortinf_asia<- a_asia %>%
  select(Esp_vida,Mortalidad_infantil) %>%
  na.omit %>% as.matrix()

bvbox(corr_esp_mortinf_asia,
      col = c("red", "blue", "green"),
      xlab = "Esperanza vida", 
           ylab = "Mortalidad Infantil",
      pch = 19, cex = 1.25)

corr_nat_mortinf_asia<- a_asia %>%
  select(Natalidad,Mortalidad_infantil) %>%
  na.omit %>% as.matrix()

bvbox(corr_nat_mortinf_asia,
      col = c("red", "blue", "green"),
      xlab = "Natalidad", 
           ylab = "Mortalidad Infantil",
      pch = 19, cex = 1.25)
...
cor.test(world_population$Esp_vida, world_population$Mortalidad, alternative = "less")
